...
export function CategorySidebar({ categories, loading }: CategorySidebarProps){
  const location = useLocation()
  const activeCat = location.pathname.startsWith('/forum/category/') ? parseInt(location.pathname.split('/').pop()||'0') : undefined
  return (
    <div className="space-y-5">
      ...
      <div className="glass-panel rounded-2xl p-4 space-y-4">
        ...
        <ul className="space-y-1.5">
          {categories?.map(c => (
            <li key={c.id}>
              <Link to={`/forum/category/${c.id}`} className={"group flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-xs transition-colors " + (c.id===activeCat ? 'bg-white/10 text-white' : 'text-slate-300 hover:bg-white/5 hover:text-white') }>
