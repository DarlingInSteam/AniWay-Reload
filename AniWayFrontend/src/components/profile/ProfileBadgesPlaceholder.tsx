    ...
    <div className="rounded-2xl bg-white/5 border border-white/10 p-5 space-y-3">
      ...
      <div className="grid grid-cols-4 gap-3">
        {Array.from({length:8}).map((_,i)=>(
          <div key={i} className="aspect-square rounded-md bg-gradient-to-br from-slate-700/60 to-slate-800/60 border border-white/10 flex items-center justify-center text-slate-500 text-xs font-medium">--</div>
        ))}
      </div>
