...
class Portals:
	...
	def chapter_parsing_start(self, title: "BaseTitle", chapter: "BaseChapter", chapter_index: int, total_chapters: int):
		"""
		Портал сообщения: начало парсинга главы.

		:param title: Данные тайтла.
		:type title: BaseTitle
		:param chapter: Данные главы.
		:type chapter: BaseChapter
		:param chapter_index: Индекс текущей главы (начиная с 1).
		:type chapter_index: int
		:param total_chapters: Общее количество глав.
		:type total_chapters: int
		"""

		self.__Logger.chapter_parsing_start(title, chapter, chapter_index, total_chapters)

	def chapter_parsing_complete(self, title: "BaseTitle", chapter: "BaseChapter", chapter_index: int, total_chapters: int, slides_count: int = 0):
		"""
		Портал сообщения: завершение парсинга главы.
		...
		:param total_chapters: Общее количество глав.
		:type total_chapters: int
		:param slides_count: Количество изображений в главе.
		:type slides_count: int
		"""

		self.__Logger.chapter_parsing_complete(title, chapter, chapter_index, total_chapters, slides_count)

	def chapter_download_start(self, title: "BaseTitle", chapter: "BaseChapter", slide_index: int, total_slides: int):
		"""
		Портал сообщения: начало загрузки изображения главы.

		:param title: Данные тайтла.
		:type title: BaseTitle
		:param chapter: Данные главы.
		:type chapter: BaseChapter
		:param slide_index: Индекс текущего изображения (начиная с 1).
		:type slide_index: int
		:param total_slides: Общее количество изображений.
		:type total_slides: int
		"""

		self.__Logger.chapter_download_start(title, chapter, slide_index, total_slides)

	def build_progress(self, title: "BaseTitle", stage: str, progress_percent: int):
		"""
		Портал сообщения: прогресс сборки архива.

		:param title: Данные тайтла.
		:type title: BaseTitle
		:param stage: Текущий этап сборки.
		:type stage: str
		:param progress_percent: Процент завершения.
		:type progress_percent: int
		"""

		self.__Logger.build_progress(title, stage, progress_percent)
...
class Logger:
	...
	def chapter_parsing_start(self, title: "BaseTitle", chapter: "BaseChapter", chapter_index: int, total_chapters: int):
		"""
		Шаблон сообщения: начало парсинга главы.
		...
		ChapterNote = "Paid chapter" if chapter.is_paid else "Chapter"
		ChapterID = chapter.id if chapter.id else f"{chapter.volume}.{chapter.number}"
		progress = f"[{chapter_index}/{total_chapters}]"
		
		self.__LogMessage(f"Title: \"{title.slug}\" (ID: {title.id}). {ChapterNote} {ChapterID} parsing started. {progress}")
		self.__PrintMessage(f"{progress} Parsing {ChapterNote} {ChapterID}...")

	def chapter_parsing_complete(self, title: "BaseTitle", chapter: "BaseChapter", chapter_index: int, total_chapters: int, slides_count: int = 0):
		"""
		Шаблон сообщения: завершение парсинга главы.
		...
		ChapterID = chapter.id if chapter.id else f"{chapter.volume}.{chapter.number}"
		progress = f"[{chapter_index}/{total_chapters}]"
		slides_info = f" ({slides_count} slides)" if slides_count > 0 else ""
		
		self.__LogMessage(f"Title: \"{title.slug}\" (ID: {title.id}). {ChapterNote} {ChapterID} parsing completed.{slides_info} {progress}")
		self.__PrintMessage(f"{progress} {ChapterNote} {ChapterID} completed.{slides_info}")

	def chapter_download_start(self, title: "BaseTitle", chapter: "BaseChapter", slide_index: int, total_slides: int):
		"""
		Шаблон сообщения: начало загрузки изображения главы.
		...
		self.__LogMessage(f"Title: \"{title.slug}\" (ID: {title.id}). Chapter {ChapterID} downloading slide {progress}")
		if slide_index % 5 == 1 or slide_index == total_slides:  # Логируем каждое 5-е изображение или последнее
			self.__PrintMessage(f"Chapter {ChapterID} downloading {progress}")

	def build_progress(self, title: "BaseTitle", stage: str, progress_percent: int):
		"""
		Шаблон сообщения: прогресс сборки архива.
		...
		self.__LogMessage(f"Title: \"{title.slug}\" (ID: {title.id}). Build stage: {stage} - {progress_percent}%")
		self.__PrintMessage(f"Building {title.slug}: {stage} - {progress_percent}%")
